...
import { GALLERY, STEPS_INFO } from '@/data/content'
...
const isUnlocked = progressStore.isStepUnlocked(2)

const handleSubmit = (code: string) => {
  error.value = ''
  success.value = ''
  
  const isValid = progressStore.validate(2, code)
  
  if (isValid) {
    success.value = 'Déverrouillé ! Découvre les visages...'
    error.value = ''
    codeInput.value = ''
  } else {
      ...
      <div class="step-page__content">
        ...
        <div v-else class="step-page__unlocked">
          <div class="step-page__intro">
            <p>
              Les visages qui comptent : toi, nous, eux...
            </p>
          </div>
