...
import { LETTER_CONTENT, STEPS_INFO } from '@/data/content'
...
const isUnlocked = progressStore.isStepUnlocked(3)

const handleSubmit = (code: string) => {
  error.value = ''
  success.value = ''
  
  const isValid = progressStore.validate(3, code)
  
  if (isValid) {
    success.value = 'Déverrouillé ! Lis le message...'
    error.value = ''
    codeInput.value = ''
  } else {
...
const letterLines = LETTER_CONTENT.split('\n').filter(line => line.trim())
...
.letter {
  ...
  &__line {
    ...
    margin-bottom: var(--spacing-md);
    opacity: 0;
    animation: fade-in-line 0.5s ease-out forwards;
