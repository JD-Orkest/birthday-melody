...
import SmsThread from '@/components/organisms/SmsThread.vue'
import InputCode from '@/components/atoms/InputCode.vue'
import { SMS, STEPS_INFO } from '@/data/content'
...
const isUnlocked = progressStore.isStepUnlocked(4)

const handleSubmit = (code: string) => {
  error.value = ''
  success.value = ''
  
  const isValid = progressStore.validate(4, code)
  
  if (isValid) {
    success.value = 'Déverrouillé ! Lis nos conversations...'
    error.value = ''
    codeInput.value = ''
  } else {
      ...
      <div class="step-page__content">
        ...
        <div v-else class="step-page__unlocked">
          <div class="step-page__intro">
            <p>
              Nos conversations, celles qui racontent notre histoire...
            </p>
          </div>
          
          <SmsThread :sections="SMS" />
        </div>
      </div>
    </div>
  </div>
